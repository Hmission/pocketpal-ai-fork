...
### 修复
- 相册历史覆盖事故根治（2026-08-21，大王发现）：`upscaleImageEntry` 补 init 守卫（对齐 generate() 模式）——此前 DRC/外部直调放大在生图页未挂载时，beginTask→persistHistory 会用空 history 覆盖 AsyncStorage 旧相册记录（8/19-8/20 共 11 条被清，图文件 100% 完好）；真机验证修复后旧记录保留+新记录追加；MASTER_LOG §64 复盘留档
