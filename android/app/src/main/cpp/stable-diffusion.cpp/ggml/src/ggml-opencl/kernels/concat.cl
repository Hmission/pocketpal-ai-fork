kernel void kernel_concat_f32(
    ...
    // 6.18 white-image fix: flattened 1D indexing. On Adreno the previous
    // group_id/local_id mapping wrote garbage from a single lane (lid=3) when
    // reading src0; deriving every coordinate from the flat global id avoids
    // the driver's work-group mapping bug entirely.
    const int idx = get_global_id(0);
    if (idx >= ne0 * ne1 * ne2 * ne3) {
        return;
    }
    const int i3 = idx / (ne0 * ne1 * ne2);
    const int i2 = (idx / (ne0 * ne1)) % ne2;
    const int i1 = (idx / ne0) % ne1;
