...
// [已裁剪 2026-08] 「ChatScreen remote vision reactivity」套件（3 例 Add image
// 启用态断言）随加号入口裁剪一并删除：大王裁定不需要图片上传，
// ChatScreen 不再传 showImageUpload={true}。组件级 gating 用例仍在
// ChatInput.test.tsx；恢复入口时（加回 prop）同步恢复本套件（见 git 历史）。
describe('ChatScreen image-attach trim guard (2026-08)', () => {
  it('does not render the Add image button even with vision capability', () => {
    runInAction(() => {
      modelStore.activeModelId = modelStore.models[0]?.id;
    });
    const {queryByLabelText} = render(<ChatScreen />, {withNavigation: true});
