 ...
 *
 * write/code 任务触发时，从已下载本地模型中选出最合适的 chat 模型自动加载。
 * 选型规则（锋利、无兜底兜圈子）：
 *   1. 已声明 capabilities 的模型优先（code→code；write→rewriting/creativity）
 *   2. 无声明则回退"非管家的最大本地模型"（越大越强）
...
const displayName = (m: Model): string => m.name || m.filename || m.fullPath || '';
