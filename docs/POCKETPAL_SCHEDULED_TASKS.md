...
| ID | 标题 | 频率 | 行为 | 状态 |
|---|---|---|---|---|
| `fd945c4a-c22e-4c68-aef8-9bfe8169fb79` | f:\pp tracked 完整性巡检 | **每小时整点**（every-hour, minute=0） | 跑 `cd f:\pp; node scripts/guard_tracked_files.js --quiet`；退出码 0 →「巡检通过」；退出码 1（无声删除）→ 列清单 + 恢复命令 + 事故警告；退出码 2（git 不可用）→ 说明降级。**每次巡检完成后必须打巡检日志**（追加到 docs/POCKETPAL_GUARD_INSPECTION_LOG.md：时间/退出码/结论/异常摘要，退出码 0 也要留痕）。除巡检与打日志外不执行任何其他操作 | ⏸ 已暂停（2026-08-30 15:06，原因：仓库开发基本完成）**⚠ 实体未停**：任务实体在 Qoder 会话层（schedule 面板）仍每小时触发，当前会话 schedule 服务不可用无法程序化停除，待可用后按 §2 操作或人工在 Qoder 面板停除 |
...
| 2026-08-27 17:29 | 行为新增「每次巡检完成后打巡检日志」（docs/POCKETPAL_GUARD_INSPECTION_LOG.md，退出码 0 也留痕） | 小黄鸡窗口（巡检会话） |
| 2026-08-30 15:06 | 暂停：状态 运行中 → 已暂停；原因：仓库开发基本完成。说明：schedule 服务不可用，会话层任务未实际停，仅在文档侧登记暂停，任务定义留档待后续处置；期间巡检日志缺新记录属正常，非漏跑 | 巡检窗口（定时巡检会话） |

## 4. 关联防线（完整防护链）

```
第 1 层（子仓分身）f:\pp pre-commit 哨兵      —— 提交即拦，已就位
第 2 层（会话层）   本表任务（每小时巡检）     —— 事故 ≤1 小时上报，已暂停（2026-08-30 15:06，仓库开发基本完成）
