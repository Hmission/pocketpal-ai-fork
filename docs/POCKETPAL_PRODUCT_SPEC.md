...
- **产品定义**：用户在模型设置页给模型打用途标签（写作/代码；玩具复用代码选型），标签随模型持久化；写作/代码任务触发模型切换时，打标签的模型最优先推荐，弹窗给出全部候选由用户单选。
- **选型优先级**（MODEL_MATRIX §1.1 单源）：用户标签命中 > 文件名指纹（入选清单）> 最大模型兜底；组内按大小降序。
- **闭环**：设置页打标签 → 选型读标签 → 弹窗多候选 → 会话级记住（不跨会话，防误记）。
