...
| 日期 | 版本 | 变更 |
|------|------|------|
| 2026-08-21 | 3.8 | Gap Ledger 补 B25（模型全平台分发闭环）：魔搭 5 仓 15.1GB + HF 3 仓双平台（Qwen×2/MiniCPM 镜像 + DreamLite ONNX + 自制 LoRA，206 字节精确匹配）；catalog 全条目双源化 + baked 钦定不装机；SOP/MODEL_MATRIX 对齐（§6.1 14→16）；token 管道固化 .env；relates 挂 MODEL_MATRIX SSOT + DEVICE_DEPLOYMENT_SOP |
