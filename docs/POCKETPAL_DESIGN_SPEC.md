...
| B15 | **存储策略与安卓规范对齐（历史债）**：模型目录双轨架构已定稿（ADR-0004）：HF 下载默认落 getExternalFilesDir（零权限、Play 合规），/Documents/AIOS/models 作为默认自定义目录继续读取（卸载不丢）；权限仅用户添加自定义目录时触发；信息存储已合规（Auto Backup + 快照） | ADR-0004 | 双轨扫描上线后验证 | ✅ 决策已定稿（待实现） |
