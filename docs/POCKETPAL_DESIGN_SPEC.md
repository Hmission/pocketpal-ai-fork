...
relates:
  [
    DRC_SPEC,
    POCKETPAL_CHAT_UI_SPEC,
    POCKETPAL_IMAGEGEN_UI_SPEC,
    POCKETPAL_UI_INTERACTION_SPEC,
    POCKETPAL_ICON_SPEC,
    ADR-0001-ui-ssot-single-source,
    ADR-0002-imagegen-header-right,
    ADR-0003-bubble-footer-unification,
    UI_GATE_VERIFICATION_SOP,
  ]
supersedes: []
---
...
**状态**：active | **版本**：3.26 | **更新**：2026-08-26
...
> 版本：v3.21（2026-08-25，全局 UI 体检治理：①基建门禁恢复——.gitattributes/全仓行尾归一，eslint CRLF 噪音 15.2 万→0（B44）；②字号可访问性红线 22 处 9/10px 清零（B45）；③PerfHistoryModal 裸 Modal 迁 Sheet 底座（B46 首项）；④Gap Ledger 补录全局混乱点批次 B44-B50（token 野档 1105/145/261、范式遗留、重复组件、布局锚点）按风险排期）
> 版本：v3.22（2026-08-25，B46/B50 执行：①存量 prettier 1,282 处全仓清零（B50 ✅）+ lint:prettier 脚本 + pre-commit 格式门禁；②弹窗范式收尾——Paper Dialog 4 处全迁 OverlayCard（ModelsResetDialog/DownloadErrorDialog/BenchmarkScreen×2）+ 休眠 components/Dialog 删除销账 + 5 套件测试同步用户存量 Alert→infoDialog 迁移；③新增 B51：eslint 规则问题 216 清理；④B46 遗留：Alert.alert ~25 处源码使用点待裁定）
> 版本：v3.23（2026-08-25，B51 执行：eslint 216 个规则问题全量清零——no-inline-styles 59（30 文件样式迁移）/ no-bitwise 42（3 文件二进制解析豁免注释）/ no-void 32（allowAsStatement 语句惯用法合法化）/ no-unused-vars 31 / no-useless-escape 18 / exhaustive-deps 13（observer 本地读桥接+依赖修正）/ unstable-nested 9（render helper 提取）/ no-shadow 6 等；**eslint . 全仓 0 错误 0 警告 + pre-commit 全量 lint 门禁**；连带完成 scripts/App.tsx/mocks 根文件格式清理与 node env 补配）
...
| token          | light   | dark    | 语义                               |
| -------------- | ------- | ------- | ---------------------------------- |
| badgeSd35      | #8E24AA | #CE93D8 | SD3.5                              |
| badgeZImage    | #00838F | #4DD0E1 | Z-Image                            |
| badgeDreamlite | #D81B60 | #F48FB1 | DreamLite                          |
| badgeFlux      | #3949AB | #9FA8DA | FLUX.2（Klein）                    |
| badgeKrea2     | #E65100 | #FFAB91 | Krea2（8-26 新增，Krea AI 品牌橙） |
...
| 域识别 | domain._ | 子页 AppBar tint、IconTile、徽章 | 域色不混用（一屏一域） |
| 状态 | semantic._ | 成功/警告/删除/信息提示 | 不做装饰 |
...
| B11  | **欢迎页插画多语言化**：宣传插画/SVG 内嵌英文文字（PhoneWithPals 等）难随语言切换；文本层已中文（默认 zh），需插画资源分语言化                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | §7                                                         | 欢迎项看不到英文文字                                                                                                                                                              | ✅ 已销账（2026-08-25 复核：PhoneWithPals.tsx 纯形状 SVG、0 Text 节点——插画无内嵌文字，前提失效） |
| B12  | **内置模型挂 HF 链接**：欢迎项/模型页内置模型挂具体 HuggingFace url（HF 下载机制已存在：HFTokenSheet + 模型行下载）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | P4 选型矩阵                                                | 模型页直接下载内置模型                                                                                                                                                            | ✅ 已销账（2026-08-25 复核：onboardingPals.ts downloadUrl 已落地，模型页可直接下载内置模型）      |
...
| B37  | **TTS 生成链路修复（2026-08-23，AUDIO_UI_SPEC v1.6，编号补录）**：TTS 生成链路（sherpa 原生）与播放链路（fork 库）对模型文件格式要求不同——下载后由 sherpaConvert.ts 生成 sherpa 格式副本（tokens.txt / unicode_indexer.bin / voice {id}.bin）；kokoro 模型需 sherpa metadata（sample_rate=24000 / n_speakers / has_espeak=1 / style_dim=510,1,256）；kitten 生成链路换官方 kitten-nano-en-v0_1（kitten_sherpa.onnx，palshub 0.8 输出纯噪声弃用）                                                                                                                                                                                                                                                                                                                                                                                                                                                              | AUDIO_UI_SPEC v1.6                                         | tsc 0 错 + 真机 TTS 生成实证                                                                                                                                                      | ✅ 已完成（2026-08-23；2026-08-25 补录本行，此前 B36 直跳 B38）                                   |
...
| B39 | **跑分演出升级与基准测试总控（大王裁定：跑分是安慰剂，过程才是药效，2026-08-24，PERF_BENCHMARK_DESIGN §10 v0.5）**：PerfMotion 统一动效引擎（AnimatedNumber 追式缓动/odometer 翻滚/揭幕动画，三页共用）；聊天页 footer 行2 + PendingIndicator 跑分感升级（不重造）；生图页 PerfPanel 折线面积图 + 全数字动效（卡片内不溢出）；基准页原地改造三用例总控台（砍祖传 bench() 合成负载/云提交链/双内存通道，用例=真实负载跨页自动导航）；四轴雷达对齐 perfScore + 跑分卡本地分享（不发公网）；6D 排查收敛零新建 store | PERF_BENCHMARK_DESIGN §10 SSOT | tsc/jest 全绿 + 五关门禁 + 真机浅深双模式 | ✅ 已完成（2026-08-25，基准套件真机全链路验证通过 + 标准负载契约三次修订，MASTER_LOG §90；B40-B43 演出层/回放/坐标轴迭代归 PERF_BENCHMARK_DESIGN 分册治理，见其 §11/§十二 与 IMAGEGEN_UI_SPEC v5.7） |

| B44 | **工程基建门禁恢复（2026-08-25，AI 体检发现）**：缺失 .gitattributes + core.autocrlf=true → 检出 CRLF、prettier 期望 LF → 全仓 eslint 15.2 万错误中 99.9%（152,443 条）为行尾噪音，质量门禁信号完全失效。修复：①新增 .gitattributes（文本统一 eol=lf + 二进制白名单 + bat/ps1 保留 CRLF）+ .editorconfig；②全仓 2,968 个跟踪文件行尾归一 LF（git 索引本就全 LF——diff 零污染；7 个误转二进制经 git hash-object 逐一对账无损恢复）；③结果：CRLF 噪音归零，eslint 从 152,659 → 1,498（真 prettier 格式 1,282 + 规则问题 216），涉及文件 1,062 → 195。遗留：存量 1,282 处真实格式差异清理挂 B50 | 无 | eslint 噪音归零 + git diff 无假变化 + tsc/jest 全绿 | ✅ 已完成（2026-08-25） |
| B45 | **字号可访问性红线（2026-08-25）**：22 处 fontSize 9/10px 违反 §2.2「最小 11（captionS）」——全量提至 11/lineHeight 14：AssistantTurnFooter/Bubble/ChatInput(2)/ToolMetricsFooter/UsageStats/ImageGenScreen(6)/HFModelSearch(3)/SquarePalCard(7)；其中 SquarePalCard 多处系「Reduced from 11px」人为降级回退 | §2.2 | tsc 0 错 + 7 套件 130 用例全绿 + git grep fontSize (9|10) 清零 | ✅ 已完成（2026-08-25；SquarePalCard 固定高度卡片需真机复核） |
| B46 | **弹窗范式收尾（§12 契约 B23 后遗留；2026-08-25 两轮执行）**：①PerfHistoryModal 裸 Modal → **已迁 Sheet 底座**（✅ 列表/回放双态 + 二级导航 + 不可见不渲染契约）；②**Paper Dialog 4 处全迁 OverlayCard**（✅ ModelsResetDialog / DownloadErrorDialog / BenchmarkScreen×2——多动作槽下沉 body 链接，OverlayCard Actions 增 testID 支持）+ **休眠 components/Dialog 删除销账**（✅ 零引用）；③**用户存量 Alert→infoDialog 迁移的测试同步**（✅ ModelsScreen/ModelFileCard/ProjectionModelSelector/ChatGenerationSettingsSheet 4 套件——存量迁移前测试全红，同步后绿）；④**Alert.alert ~25 处源码使用点**（ChatInput 5/HeaderRight 4/EmbeddedVideoView 2/ContentReportSheet 3/ModelErrorReportSheet 2/ModelsScreen 2/ModelFileCard 2/ProjectionModelSelector 2/PalGenerationSettingsSheet 1 等，多按钮确认与通知语义）——统一迁移或登记豁免需大王裁定；⑤ChatPalModelPickerSheet/SearchView Sheet 直连旁路评估 | §12.1-12.4 | 底部弹层零裸 Modal（✅）；Paper Dialog 零残留（✅）；Alert.alert 全量裁定/清零（④） | 🟡 执行中（①②③✅，④待大王裁定） |
| B47 | **token 野档清理（全库体检数据）**：间距裸数字 1,105 处（~220 无档位 6/10/14/3/18/28 + ~760 数值对但写法裸）+ 圆角裸数字 145（~45 无档 5/18/15/6/10/3）+ 字号裸数字 239 处（12/13 档对齐，B45 已清 9/10）+ 颜色 16 处（EmbeddedVideoView 遮罩族 8/代码高亮主题 3/其余 5，反推紫等已登记除外）。重点：ImageGenScreen styles 163、ModelCard 65、TTSSetupSheet 61、ChatInput 58；**整文件裸奔** DatabaseInspectorScreen（47 vs token 1）/ EmbeddedVideoView（38 vs 2）——新建样式禁直写数字，间距/圆角无档位先补 token 再引用 | 补档位表 + §1.6/§2.2 映射 | 体检口径下裸数字归零（豁免表登记） | 🔵 执行中 |
| B48 | **重复组件收敛（全库体检数据）**：①三点加载动画三合一——LoadingBubble 与 PendingIndicator.Dot 逐字双胞胎 + useWaveDots 第三套（7 处复用）；②chip 胶囊样板收敛 ui/Chip（ImageTaskActions vs TaskErrorCard 整套复制，ui/Chip 现有零引用）；③进度条统一（ResultPreview/ImageTaskProgress/ModelFileCard/BannerBar 四套自绘 + paper ProgressBar 混用）；④toggle/滑块 4 形态（workshopSlider 孤例）；⑤spinner 三来源（paper/RN/自绘 CircularActivityIndicator） | B48 定稿 | 同一模式单一实现，废弃实现删除并登记 | 🔵 执行中 |
| B49 | **布局安全锚点（全库体检数据）**：①全屏查看器锚点族收口——TextMessage top:50/bottom:56 与 ChatView bottom:56 两文件重复写、无 safe-area（挖孔屏/手势条/字体缩放错位）；②DownloadOverlay left:50/right:45 不对称条带（分屏/RTL 歪斜）；③EmbeddedVideoView bottom:180 三层 absolute 混排（180/11%/5%）；④OnboardingScaffold topLeftSlot 裸 16 与同族 topInset+ 不一致 | B49 定稿 | 查看器族统一安全区；不对称/混排锚点消除 | 🔵 执行中 |
| B50 | **存量 prettier 格式化清理 + 门禁固化（2026-08-25 ✅）**：全仓 src prettier --write 清零 1,282 处格式差异（195 文件），eslint 仅剩 216 个规则问题（真问题，不属格式）；新增 `lint:prettier` 脚本（prettier --check src）+ **pre-commit 格式门禁挂载**（husky pre-commit 追加 lint:prettier，commitlint/worktree guard 保留）；eslint 全量入 pre-commit 待 B51 | B44 完成 | prettier 全仓零错误 + pre-commit 拦截格式回归 | ✅ 已完成（2026-08-25） |
| B51 | **eslint 规则问题清理（2026-08-25 ✅）**：216 个真实问题全量清零——①no-inline-styles 59（30 文件内联样式迁移至 styles，动态样式走「基+态」键/组件内变量）；②no-bitwise 42（PerfAreaChart/pngUtil/sherpaConvert 二进制解析文件级豁免注释）；③no-void 32（`.eslintrc` allowAsStatement——fire-and-forget 语句惯用法）；④no-unused-vars 31 / no-useless-escape 18 / no-shadow 6 / exhaustive-deps 13（observer 本地读桥接、latest-ref、表达式提取）/ unstable-nested 9（模块级 render helper）/ 其余 7；⑤连带：scripts/App.tsx/根 mock 格式与 node env（es2021）补配。**结果：eslint . 全仓 0 错误 0 警告 + pre-commit 挂 eslint 全量门禁** | B50 完成 | eslint src 零错误零警告 + pre-commit 全量 lint | ✅ 已完成（2026-08-25，142 套件 1549 用例全绿） |
| B52 | **自登记功能缺口收尾（2026-08-26 ✅）**：①图标尺寸收口 12 处 `size={20/24}` → `theme.iconSize.m/l`（✅ grep 零残留）；②长按菜单「全文朗读」（✅ ChatView speak 项 + l10n 三语）；③**确认型/信息型 Alert 迁移**（✅ 两子批完成）：信息型 19 处 → infoDialog（HeaderRight/PalHeaderRight/EmbeddedVideoView/AuthSheet 9/PalDetailSheet/ContentReportSheet/SidebarContent/ChatInput 5/exportUtils 2/ModelErrorReportSheet 2/PalGenerationSettingsSheet/DevTools 等）+ 二元确认 17 处 → confirmDialog（MemoryScreen 3/ModelDirs/ModelFileCard 2/ProjectionModelSelector 2/ProfileSheet/VoicePicker/DevTools/GenerationSettings/SquarePalCard 3/ServerDetails 等），**测试 12+ 套件全量同步**；**特殊保留（登记）**：ModelsScreen 148 服务器多选列表 / 226 文件三选一 / ModelStore 2087（用户并行窗口在途文件）/ exportUtils 343·400 与 androidPermission 152（带动作按钮）/ safeAlert 封装——多选与带动作语义非二元弹窗，专项迁移待定 | §7/§12 | 商标承诺全达：grep 零残留、朗读可达、Alert 可迁处全迁；中 67 处源码使用点 → 剩 ~8 处特殊保留 | ✅ 已完成（信息型+确认型；特殊保留挂 B55 专项） |
...
- **生图页例外（2026-08-21 大王裁定登记，B30 复查；v4.3 改造）**：生图页轻提示全量迁移**预览卡片顶部横幅**（§12.3 BannerBar overlay 形态：语义色 12% wash 直接透出无瓷底 + radius.m 圆角容器 + elevation 4，absolute 叠预览卡片图区顶部，只压预览卡片不压历史区/创作区）——底部 Snackbar 灰色底 + 遮挡底部按钮，在生图页停用；其余页面维持底部 Snackbar。瞬时反馈 3s 自动消失 + 整卡点击关闭；状态引导（编辑锁定）常驻至状态结束且不可点关闭。详见 IMAGEGEN_UI_SPEC §9。
...
| 日期       | 版本 | 变更                                                                                                                                                                                                                                                                                                                           |
| ---------- | ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 2026-08-26 | 3.25 | **B52③ 执行**：Alert 迁移两子批完成（信息型 19 → infoDialog / 二元确认 17 → confirmDialog，测试 12+ 套件同步）；特殊保留 8 处登记（多选/带动作/用户在途）；B53 modelCatalog 在途确认（Krea2 并行窗口准入）；B54 评估完成（联动在编排层，待并行窗口收口实施）                                                                   |
| 2026-08-26 | 3.24 | **CHAIN_AUDIT_20260826 执行**：Gap Ledger 挂 B52-B54（图标收口 12 处 ✅/长按朗读 ✅/确认型 Alert 挂批/红测试/架构归一）；invariants 白名单补 DownloadErrorDialog（B46 引入缺口自纠）；v3.24 起挂账必挂批次号                                                                                                                   |
| 2026-08-25 | 3.23 | **B51 执行**：eslint 216 规则问题全量清零（inline-styles/bitwise/void/unused/escape/hooks/nested/shadow 等）+ eslint . 全仓 0 错 + pre-commit 全量 lint 门禁；连带 scripts/App.tsx/mocks 根文件清理与 node env 补配                                                                                                            |
| 2026-08-25 | 3.22 | **B46/B50 执行**：存量 prettier 1,282 处全仓清零 + lint:prettier 脚本 + pre-commit 格式门禁（B50 ✅）；Paper Dialog 4 处全迁 OverlayCard + 休眠 components/Dialog 删除 + 4 套件测试同步用户存量 Alert→infoDialog 迁移（B46 ①②③✅，④ Alert~25 处待大王裁定）；新增 B51（eslint 规则问题 216）；relates 挂 IMAGEGEN_UI_SPEC v4.3 |
