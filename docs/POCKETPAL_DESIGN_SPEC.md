...
| B29 | **模型加载卡片动效统一（大王反馈，MASTER_LOG §66）**：聊天页两处模型加载卡片升级三点波浪 + 2% 底条动效——①顶栏选择器 ChatPalModelPickerSheet 加载卡片（原纯文本「正在加载 · 已耗时 Xs」）；②任务切换弹窗 ModelSwitchDialog 加载态（原 ActivityIndicator 转圈，移除统一设计语言）；复用 useWaveDots（JS driver 合规 07a47ed）与生图任务卡 ImageTaskProgress 同款；测试 mock 补 radius.xxs/colors.shadow | CHAT_UI_SPEC §16.2/§18.7 v4.7 + UI_INTERACTION_SPEC v1.3 | tsc 0 错 + 3 套件 17 用例全绿 | ✅ 已完成（2026-08-21，待真机目视） |
