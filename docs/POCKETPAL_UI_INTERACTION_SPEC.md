...
- **加载态动效（v1.3）**：三点波浪 + 2% 底条（生图任务卡同款，useWaveDots 复用），替代 ActivityIndicator 转圈——遮罩阻塞期动效提示加载进行中，用户不误判卡死；顶栏选择器加载卡片同款升级（CHAT_UI_SPEC §16.2/§18.7）；候选行/加载态模型名改中文简称（getModelDisplayNameWithParams 与选择器同源，不显示原始文件名）
