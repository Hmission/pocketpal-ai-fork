...
- 两台测试机均 16GB RAM，但 **GPU 分配能力差异巨大**：
  ...
  - **跑分错位定论（2026-08-24，REDMI K Pad 正名）**：天玑9400+ 理论跑分顶级但生图比 Adreno 840 慢 4~5 倍（~156 vs ~32 秒/步）——跑分测 CPU/游戏光栅化，生图走 OpenCL 通用计算；Mali 无 qcom 私有扩展/XMEM 零拷贝/fp16 厂商内核（被迫 fp32 通用路径）+ 驱动生态弱，属结构性生态税非链路 bug
  ...
  - **FLUX.2 Klein 平板准入（2026-08-25，§91）**：四线侦察（门控/引擎/内核/内存）后链路全通——gpuPolicy 声明式矩阵（废 requiresHighGpu）+ q4_0 l4_lm 半精度变体 + ``params_backend="te=disk"`` 解 TE+DiT 同驻留超配额（7.83GB→存活）；全链 296.77s、采样 35.4 s/步、nan=0；Z-Image 维持三重锁（6.9GB 硬顶 + 740 hang + GDN 断言）不打
  - **FLUX.2 Klein 马赛克根因终局（2026-08-27，§99 B63）**：13 组筛变量对照证实——量化（Q4_0/Q4_K_M/Q4_K_S 全纹理）、内核变体（fp32 通用/fp16-tiled 均坏且 fp16 更糟：G 通道死/品红）、VAE（DiT=OpenCL+VAE=CPU 仍纹理）、治理组（DISABLE_ADRENO/XMEM 开关无关）全排除；CPU（桌面+端侧）干净、Z-Image（FLUX.1）OpenCL 干净 → **根因 = vendored ggml-opencl 通用内核对 FLUX.2 张量 shape 的 bug**（txt_in K=7680 / img_in K=128 / 32 通道 patchify 嫌疑），应用层无绕过。落地：klein 默认后端切 CPU（正确、慢 ~5h）、保持 experimental、JNI 回 8-25 门控；**内核修复专项立项**（移动端 OpenCL 逐 op tensor instrumentation + 上游级补丁）
