...
| DeepSeek Hermes | **未选型** | 无任何选型记录（2026-08-14 并行窗口安装中，**立即叫停**） |
...
## 5. 真机现状（2026-08-14 装机后）

- LLM 已装：§1 全 7 件 ✓ + 遗留 `qwen3_06b_q8_0.gguf`（待大王定夺去留）
- 生图已装：SDXL fp16（待清理，淘汰）+ SD3.5 四件套 ✓ + Z-Image 三件套 ✓ + DreamLite 内置 ✓
- 清理待办（大王批准后执行）：删 SDXL fp16 6.9GB；0.6B 去留待定
