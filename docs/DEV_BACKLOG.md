...
| # | 项 | 目标 | 前置 | 验收标准 | 归属 |
|---|---|---|---|---|---|
| 19 | **上下文策展极限化** | 策展表重排（4B→16384/2B→32768/1B→32768 等）+ 设备感知预算 `resolvePssSafeBudget`（可用内存口径上限 9GB）+ `CURATED_TABLE_VERSION` 版本化拉齐 + `recommendNCtx` 死代码删除 | 无 | tsc 0 错 + jest 全绿 + K90 真机各档加载 PSS 实测 ≤ 设备预算 | **2026-08-26 落码**（待真机） |
| 20 | **压缩大块化** | `COMPACT_TARGET_WATERMARK=0.5`（触发 0.8 → 压到半水位）+ 摘要输入预算上限 6000→12000 + `TOOL_BASELINE_TOKENS=4500` 常量与策展断言 | 项 19 | 16K 档连续 10+ 轮触顶仅 1 次压缩；摘要一次吃下压到半水位的 slice | **2026-08-26 落码**（待真机） |
