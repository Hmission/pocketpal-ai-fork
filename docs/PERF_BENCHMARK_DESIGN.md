...
> 状态：**v1.2 生图页面板四修落地（去 SoC 小字 / 弃灰底改 hairline 分隔 / 卡片加宽缩字 / 默认叠全）** | 版本：1.2 | 2026-08-25
...
- **坐标系**（PerfAreaChart 新 props `axes`，默认关，聊天页复用零回归）：左 Y 刻度 4 档按维度单位（PSS→GB / 负载→%/温度→°C/功耗→W）+ 底部 X 时间刻度（1Hz 采样索引差=相对秒，不编造）+ 水平/垂直网格（hairline 淡）；阈值虚线端点加迷你标注 **5GB/6GB**——「虚线是什么意思」可读化（大王疑问实答：紫实线=功耗 #BA68C8，橙虚线=5GB 逼近线，红虚线=6GB 硬杀线）。
