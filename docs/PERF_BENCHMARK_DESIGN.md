...
| 日期 | 版本 | 变更 |
|------|------|------|
| 2026-08-27 | 1.3 | **UI 一致性升级（大王五问裁定）**：①**叠图迷你遥测条**——新建共享组件 `PerfMiniRow`（折叠头「性能 ▾ + PSS 大字阈值色 + 内存/CPU/温度胶囊分级色」+ 展开体 B43 迷你折线：PSS+CPU 双线/温度热力带/坐标轴/5·6GB 标注/vivid），反推/编辑/放大叠图 overlay 挂载（默认折叠一行少遮挡原图），PendingIndicator 遥测行+图表迁移共用（testIDPrefix 兼容既有断言）；②**聊天页 B43 同步**——PendingIndicator 折线 44→56pt 双线+热力带+坐标轴+演出层，AssistantTurnFooter 展开层加 axes/tempBand/vivid（72pt）；③**容器自适应**——taskPage 去 overflow:hidden + 顶对齐 + minHeight 240，根治 PerfPanel 默认展开时内容超高双向裁切（顶部 WaveDots 被切）；④阈值/格式器抽取 `utils/perfTiers` 单一事实源（PerfPanel/PerfMiniRow/Footer 三方共用）；⑤顶部横幅去灰——BenchmarkHudBar 灰底 → 跑分金 brandAccent 12% wash + 文案居中（见 CHAT_UI_SPEC banner 治理）。门禁：tsc 0 错 + 受影响 7 套件 178 用例全绿；待真机验证 |
