...
| 日期 | 版本 | 变更 |
|------|------|------|
| 2026-09-03 | 1.5 | **聊天页跑分链路修复（A1+B1，task-47d 窗口）**：①**PerfMiniRow 单位换算修复**——PSS KB→GB 与大字/内存胶囊两处注入面补 /1024/1024（与 PerfPanel/Footer 峰值/PerfHistoryModal 同口径），根治「16G 内存显示几百 G」类位数爆掉（2026-08-27 ae26403 重构引入即错，gbTinyFmt 保留纯格式化器）；②**生成期采样单源**——PendingIndicator 删除自建 1Hz NativeHardwareInfo 采样，改为订阅 chatTurnPerf.livePoints（生成期全 App 唯一采样源，消除双 1Hz 叠加开销）；③新键 chatTurnPerf.livePoints 只读轨迹 getter（运行外诚实返空）。门禁：tsc 0 错 + PerfMiniRow 5 例单测 + AssistantTurnFooter 峰值换算哨兵断言全绿；K90 真机对照（覆盖安装后）：TTFT 73007→38614ms（-47.1%，A2 工具裁剪/思考关闭联合效果） |
