...
| 2026-08-27 | 2.5 | 管家扩写根治「鹦鹉学舌」（promptWriter v4）：真机报障画苹果出提示词是海边美女——根因 2.4 示例内嵌 system 纯文本被 1B 短输入复述；修复：示例抽 SSOT 常量改 user/assistant 示范轮（续写位置正确）+ temperature 0.7→0.4 + isParrotingExample 检测闸（词重合 ≥70% 判定复读显式抛错，回退原文标「提示词未增强」）；单测 5 用例 + chatImageTask 9 用例全绿（详见内部 MASTER_LOG §101） |
